...
def main():
    ...
    # utf-8-sig 同时兼容带 BOM 与不带 BOM 的正文，并移除正文前的 BOM。
    content = open(src, encoding="utf-8-sig").read().strip()
    ...
    # 预览页本身已有 meta charset；额外写入 BOM 可兼容 Safari 本地文件识别。
    open(out, "w", encoding="utf-8-sig").write(out_html)
